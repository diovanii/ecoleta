...
const CreatePoint = () => {
  ...
  useEffect(() => {
    axios.get<IBGEUFResponse[]>('https://servicodados.ibge.gov.br/api/v1/localidades/estados').then(response => {
      const ufInitials = response.data.map(currentUf => currentUf.sigla)
  ...
  useEffect(() => {
    ...
    axios.get<IBGECityResponse[]>(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${selectedUf}/municipios`).then(response => {
      const cityNames = response.data.map(currentCity => currentCity.nome)
  ...
  function handleSelectItem(id: number) {
    const alreadySelected = selectedItems.findIndex(currentItem => currentItem === id)

    if (alreadySelected >= 0) {
      const filteredItems = selectedItems.filter(currentItem => currentItem !== id)
          ...
          <div className="field-group">
            <div className="field">
              ...
              <select onChange={ handleSelectUf } value={ selectedUf } name="uf" id="uf">
                ...
                { ufs.map(currentUf => (
                  <option key={ currentUf } value={ currentUf }>{ currentUf }</option>
                )) }
              </select>
            </div>
            ...
            <div className="field">
              ...
              <select onChange={ handleSelectCity } value={ selectedCity } name="city" id="city">
                ...
                { cities.map(currentCity => (
                  <option key={ currentCity } value={ currentCity }>{ currentCity }</option>
                )) }
              </select>
            </div>
          </div>
        ...
        <fieldset>
          ...
          <ul className="items-grid">
            { items.map(currentItem => (
              <li onClick={ () => handleSelectItem(currentItem.id) }
                className={ selectedItems.includes(currentItem.id) ? 'selected': '' }
                key={ currentItem.id }
              >
